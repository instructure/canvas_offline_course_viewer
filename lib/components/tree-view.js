...
function createTreeItemStructure (structure, currentPath, callbacks, refCallback) {
  if (!structure.expanded) return []

  return structure.children.map((child, index) => {
    const childPath = currentPath.concat([index])
    const boundRefCallback = (treeItem) => refCallback(childPath, treeItem)
    return <TreeItem
      {...callbacks}
      ...
      path={childPath}
      >
      {createTreeItemStructure(child, childPath, callbacks, refCallback)}
    </TreeItem>
  })
}
...
export default class TreeView extends React.Component {
  ...
  _nextVisibleTreeItem (path) {
    ...
    if (child.props.structure.children.length > 0) {
  ...
  _nextVisibleSiblingOrParentItem (path) {
    ...
    // next sibling?
    if (childIndex < parent.props.structure.children.length - 1) {
  ...
  _lastDeepestVisibleChildItem (path) {
    ...
    while (treeItem && treeItem.props.structure.children.length > 0) {
      const lastChildIndex = treeItem.props.structure.children.length - 1
  ...
  _keyboardCollapseNode (path) {
    ...
    if (treeItem.props.structure.children.length === 0) {
  ...
  _keyboardExpandNode (path) {
    const treeItem = this._getTreeItem(path)
    if (treeItem.props.structure.children.length === 0) return
