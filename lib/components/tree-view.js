...
function createTreeItemStructure (structure, callbacks, refCallback) {
  ...
  return structure.files.reduce((acc, child, index) => {
    if (child.type !== 'folder') return acc
    const boundRefCallback = (treeItem) => refCallback(child.path, treeItem)
    acc.push(<TreeItem
      {...callbacks}
      ...
      structure={child}
      >
      {createTreeItemStructure(child, callbacks, refCallback)}
    </TreeItem>)
    return acc
  }, [])
}
