...
export default class TreeItem extends React.Component {
  ...
  handleFocus (e) {
    e.stopPropagation()
    this.props.onNewFocusedPath(this.props.path)
  }

  handleButtonClick (e) {
    e.stopPropagation()
    this.props.onButtonClick(this.props.path)
  }
  ...
  _ariaExpanded () {
    ...
    if (this.props.structure.children.length) {
  ...
  _itemTabIndex () {
    return this.props.isFocusedPath(this.props.path) ? '0' : '-1'
  }
  ...
  _renderItemText () {
    return <span className="treeview__text">
      <Link tabIndex="-1">{this.props.structure.label}</Link>
    </span>
  }
  ...
  render () {
    const itemClasses = {
      treeview__item: true,
      treeview__item__leaf: this.props.structure.children.length === 0,
    }

    const itemLabel = I18n.t('%{label}, level %{treelevel}', {
      label: this.props.structure.label,
      treelevel: this.props.path.length,
    })
...
TreeItem.propTypes = {
  ...
  // list of indexes to find this node in the overall structure tree. used for events.
