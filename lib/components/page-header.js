    ...
    return <div className='page-header'>
      <div className='page-header__breadcrumb'>
        <Breadcrumb label='navigation'>
          <BreadcrumbLink href="index.html">{this.props.course.title}</BreadcrumbLink>
          <BreadcrumbLink>{this.props.breadcrumb || ''}</BreadcrumbLink>
        </Breadcrumb>
      </div>
      <span className='page-header__last-download'>
        <Typography weight="light">{I18n.t('Last Export:')} {this.props.course.lastDownload}</Typography>
      </span>
    </div>
  }
}
