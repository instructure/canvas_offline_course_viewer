...
describe('files', () => {
  ...
  it('should invoke callback when file is clicked', () => {
    const files = [{type: 'file', name: 'file.txt', size: 4, files: null}]
    const callback = jest.fn()
    const wrapper = enzyme.mount(<FilesList files={files} onFolderClick={() => {}} onFileClick={callback} />)
    wrapper.find('Link').simulate('click')
    expect(callback).toHaveBeenCalledTimes(1)
